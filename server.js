...
/**
 * 分析表格内容：解析 CSV/TSV，得到带 日期、角色、差值 的记录列表，并汇总总计、按日、按角色。
 * 表头需含：等级、开始经验、结束经验、角色名称；可选 日期、差值（有差值列则优先用，否则用等级/开始/结束计算）。
 * 返回带 detectedHeaders 便于排查 0 条记录问题。
 */
function analyzeTableContent(text) {
  const { headers, rows } = parseTable(text);
  const idxLevel = headers.findIndex((h) => /等级/.test(h));
  let idxExpStart = headers.findIndex((h) => /经验.*开始|开始.*经验|经验值\s*[（(]?\s*开始|起始.*经验|经验.*起始/.test(h));
  if (idxExpStart < 0) idxExpStart = headers.findIndex((h) => /^开始$|^开始经验$|^起始经验$/.test(h) || /开始|起始/.test(h));
  ...
  for (const row of rows) {
    ...
    if (idxWeekday >= 0 && (row[idxWeekday] || '').trim()) {
      const weekdayStr = String(row[idxWeekday] || '').trim();
      let year = defaultYear, month = defaultMonth, dayHint = null;
      if (date && /^D(\d{2})$/.test(date)) dayHint = parseInt(date.slice(1), 10);
      else if (date && /^\d{4}-\d{2}-\d{2}$/.test(date)) {
        year = parseInt(date.slice(0, 4), 10);
        month = parseInt(date.slice(5, 7), 10);
        dayHint = parseInt(date.slice(8, 10), 10);
      } else if (date && idxDate >= 0) {
        const raw = String(row[idxDate] || '').trim();
        const num = parseInt(raw, 10);
        if (!Number.isNaN(num) && num >= 1 && num <= 31) dayHint = num;
      }
      if (dayHint == null && date && /^\d{4}-\d{2}-\d{2}$/.test(date))
        dayHint = parseInt(date.slice(8, 10), 10);
      const resolved = resolveDateByWeekday(year, month, dayHint || 1, weekdayStr);
...
function saveSmartTable(rows) {
  try {
    if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
    fs.writeFileSync(SMART_TABLE_FILE, JSON.stringify({ rows, updatedAt: new Date().toISOString() }, null, 0), 'utf8');
  } catch (e) {
    console.error('[smart-table] save error', e.message);
  }
}

/** 去重时统一日期：D12 与 2026-03-12 视为同一天；结果做有效日期钳位（如 2-30→2-28） */
...
/** 完全一样的行只保留第一条（自动去重）；日期统一为 YYYY-MM-DD 再比，避免 D12 与 2026-03-12 重复；保留的行日期会规范为 YYYY-MM-DD */
function dedupeSmartTableRows(rows) {
  ...
  return rows.filter((r) => {
    ...
    seen.add(key);
    if (dateKey && String(r.date || '').trim() !== dateKey) r.date = dateKey;
    return true;
  });
}

/** 用服务器智能表格的 rows 重算并写入 analyzed；先统一无效日期（如 2-30→2-28）、去重，有变更则保存 */
function rebuildAnalyzedFromSmartTable() {
  let rows = loadSmartTable();
  if (rows.length === 0) return;
  let changed = false;
  rows = rows.map((r) => {
    const d = r.date != null ? String(r.date).trim() : '';
    const fixed = /^\d{4}-\d{1,2}-\d{1,2}$/.test(d) ? clampToValidDateStr(d) : d;
    if (fixed !== d) { changed = true; return { ...r, date: fixed }; }
    return r;
  });
  const before = rows.length;
  rows = dedupeSmartTableRows(rows);
  if (rows.length < before) changed = true;
  if (changed) {
    saveSmartTable(rows);
    if (rows.length < before) console.log('[smart-table] 自动去重并修正无效日期，删除', before - rows.length, '条重复');
    else console.log('[smart-table] 已修正无效日期（如 2-30→2-28）');
  }
  let totalDiff = 0;
  const byDate = {};
  const byRole = {};
  ...
  saveAnalyzed({
    ...
    byDate,
    byRole,
    detectedHeaders: ['日期', '角色名称', '等级', '经验值(开)', '经验值(结束)', '差值'],
    rawRowCount: rows.length,
  });
}
  ...
  try {
    ...
    if (date) {
      ...
      records = records.filter((r) => {
        const d = (r.date || '').trim();
        return /^\d{4}-\d{2}-\d{2}$/.test(d) && d >= dateFrom && d <= dateTo;
      });
    } else {
...
/** 实时查看服务器智能表格（只读）；管理员在页面上查看机器人写入的原始数据 */
app.get('/api/smart-table', (req, res) => {
  try {
    const rows = loadSmartTable();
    let updatedAt = '';
    try {
      if (fs.existsSync(SMART_TABLE_FILE)) {
        const raw = fs.readFileSync(SMART_TABLE_FILE, 'utf8');
    ...
    res.json({ rows, updatedAt });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/** 管理员编辑智能表格某一行（按行号 0 起）；body 可只传要改的字段 */
app.patch('/api/smart-table/row/:index', requireAdmin, express.json({ limit: '16kb' }), (req, res) => {
  try {
    ...
    const rows = loadSmartTable();
    if (index >= rows.length) return res.status(404).json({ error: '该行不存在' });
    const b = req.body || {};
    ...
    if (b.roleName !== undefined) row.roleName = String(b.roleName).trim() || row.roleName || '未知';
    if (b.diff !== undefined) { const num = parseNum(b.diff); row.diff = Number.isFinite(num) ? num : row.diff; }
    if (b.expStart !== undefined) row.expStart = b.expStart !== null ? b.expStart : '';
    if (b.expEnd !== undefined) row.expEnd = b.expEnd !== null ? String(b.expEnd).trim() : '';
    saveSmartTable(rows);
    rebuildAnalyzedFromSmartTable();
    res.json({ ok: true, message: '已更新', row: rows[index] });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.delete('/api/smart-table/row/:index', requireAdmin, (req, res) => {
  try {
    ...
    saveSmartTable(rows);
    rebuildAnalyzedFromSmartTable();
    res.json({ ok: true, message: '已删除', recordCount: rows.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/** 机器人回调：写入一行到服务器智能表格，并立即重算分析结果（可分析表格实时更新） */
app.post('/api/smart-table/row', requireSmartTableSecret, express.json({ limit: '64kb' }), (req, res) => {
  try {
    ...
    const row = {
      ...
      expStart: expStart !== undefined && expStart !== null ? expStart : '',
      expEnd: expEnd !== undefined && expEnd !== null ? String(expEnd).trim() : '',
    };
    const rows = loadSmartTable();
    rows.push(row);
    saveSmartTable(rows);
    rebuildAnalyzedFromSmartTable();
    console.log('[smart-table] 新增 1 条，共', rows.length, '条');
    res.json({ ok: true, message: '已写入智能表格并更新分析', recordCount: rows.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/** 管理员批量导入历史记录到智能表格（粘贴 CSV/TSV 或传 rows 数组），导入后机器人可继续追加 */
app.post('/api/smart-table/import', requireAdmin, express.json({ limit: '6mb' }), (req, res) => {
  try {
    const body = req.body || {};
    let toAppend = [];
    if (body.text && typeof body.text === 'string') {
      const analyzed = analyzeTableContent(body.text.trim());
      if (!analyzed.rows || !analyzed.rows.length) {
        return res.status(400).json({
          error: '未能解析出有效记录。请确认表头含：等级、经验开始、经验结束、角色名称；或有「差值」列。',
          detectedHeaders: analyzed.detectedHeaders,
        });
      }
      toAppend = analyzed.rows.map((r) => ({
        date: r.date || '',
        roleName: r.roleName || '未知',
        diff: r.diff != null ? r.diff : 0,
        expStart: r.expStart !== undefined && r.expStart !== null ? r.expStart : '',
        expEnd: (r.expEnd !== undefined && r.expEnd !== null) ? String(r.expEnd).trim() : '',
      }));
    } else if (Array.isArray(body.rows) && body.rows.length > 0) {
      const todayStr = new Date().toISOString().slice(0, 10);
      for (const b of body.rows) {
        let date = (b.date != null) ? String(b.date).trim() : '';
        if (date) { const n = normalizeDateCell(date); if (n) date = n; }
        if (!date) date = todayStr;
        const roleName = (b.roleName != null ? String(b.roleName).trim() : '') || (b.角色名称 != null ? String(b.角色名称).trim() : '') || '未知';
        const diff = parseNum(b.diff ?? b.差值);
        toAppend.push({
          ...
          roleName: roleName || '未知',
          diff: Number.isFinite(diff) ? diff : 0,
          expStart: b.expStart ?? b['经验值(开)'] ?? '',
          expEnd: (b.expEnd ?? b['经验值(结束)'] ?? '') !== undefined && (b.expEnd ?? b['经验值(结束)']) !== null ? String(b.expEnd ?? b['经验值(结束)']).trim() : '',
        });
      }
    } else {
    ...
    saveSmartTable(rows);
    rebuildAnalyzedFromSmartTable();
    console.log('[smart-table] 导入', toAppend.length, '条，共', rows.length, '条');
    res.json({ ok: true, message: '已导入到智能表格并更新分析', imported: toAppend.length, recordCount: rows.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

/** 管理员上传文件批量导入到智能表格（同分析表格：支持 .csv/.txt/.xlsx） */
app.post('/api/smart-table/import-upload', requireAdmin, (req, res, next) => {
  upload.single('file')(req, res, (err) => {
    ...
    try {
      ...
      const analyzed = analyzeTableContent(text.trim());
      if (!analyzed.rows || !analyzed.rows.length) {
      ...
      const toAppend = analyzed.rows.map((r) => ({
        date: r.date || '',
        roleName: r.roleName || '未知',
        diff: r.diff != null ? r.diff : 0,
        expStart: r.expStart !== undefined && r.expStart !== null ? r.expStart : '',
      ...
      saveSmartTable(rows);
      rebuildAnalyzedFromSmartTable();
      console.log('[smart-table] 文件导入', toAppend.length, '条，共', rows.length, '条');
      sendJson(res, 200, { ok: true, message: '已导入到智能表格并更新分析', imported: toAppend.length, recordCount: rows.length });
    } catch (e) {
...
/** 一次性写入多行（可选）；body.rows 为数组，每项同单条格式 */
app.post('/api/smart-table/rows', requireSmartTableSecret, express.json({ limit: '10mb' }), (req, res) => {
  try {
    ...
    saveSmartTable(rows);
    rebuildAnalyzedFromSmartTable();
    console.log('[smart-table] 批量写入', list.length, '条，共', rows.length, '条');
    res.json({ ok: true, message: '已写入并更新分析', recordCount: rows.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/callback', async (req, res) => {
  try {
    ...
    try {
      ...
      // 写入服务器智能表格，供网页实时查看与分析（用中国时区日期，避免服务器时区错误写成 3-20）
      const chinaTz = 'Asia/Shanghai';
      const now = new Date();
      const y = parseInt(new Intl.DateTimeFormat('zh-CN', { timeZone: chinaTz, year: 'numeric' }).format(now), 10);
      const m = parseInt(new Intl.DateTimeFormat('zh-CN', { timeZone: chinaTz, month: '2-digit' }).format(now), 10);
      ...
      const smartRow = {
        date: dateStr,
        roleName: row.roleName || '未知',
        diff: row.diff != null ? row.diff : 0,
        expStart: row.expStart !== undefined && row.expStart !== null ? row.expStart : '',
        expEnd: row.expEnd !== undefined && row.expEnd !== null ? String(row.expEnd) : '',
      };
      const rows = loadSmartTable();
      rows.push(smartRow);
      saveSmartTable(rows);
      rebuildAnalyzedFromSmartTable();
      console.log('[callback] 已写入服务器智能表格，共', rows.length, '条');
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      if (rows.length < before) {
        saveSmartTable(rows);
        rebuildAnalyzedFromSmartTable();
        console.log('[smart-table] 启动时去重，删除', before - rows.length, '条重复');
      }
    }
  } catch (e) {
    console.error('[smart-table] 启动去重失败', e.message);
  }
