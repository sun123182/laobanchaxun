 ...
 * 日期只以「日期」这一列为准，尽量按常见格式规范成 YYYY-MM-DD；
 * 实在识别不了就保留原文本，不再根据「星期」去推断或修改日期，
 * 确保和上传表格里看到的日期尽量一致。
 */
function analyzeTableContent(text) {
  ...
  for (const row of rows) {
    ...
    if (rawDate !== null && String(rawDate).trim() !== '') {
      const normalized = normalizeDateCell(rawDate);
      // 能识别出标准日期或 Dxx 就用规范值，否则保留原文本
      date = normalized || String(rawDate).trim();
    }
...
/** 管理员批量修改日期：把智能表格里某个日期文本统一替换成另一个日期文本，并重建分析结果 */
app.post('/api/smart-table/bulk-update-date', requireAdmin, express.json({ limit: '4kb' }), (req, res) => {
  try {
    const fromRaw = req.body && req.body.from != null ? String(req.body.from).trim() : '';
    const toRaw = req.body && req.body.to != null ? String(req.body.to).trim() : '';
    if (!fromRaw || !toRaw) {
      return res.status(400).json({ error: '请同时填写「原日期」和「改为」' });
    }
    const data = loadSmartTable();
    const idxDate = getHeaderIndex(data.headers, /日期/);
    if (idxDate < 0) return res.status(400).json({ error: '当前智能表格中没有「日期」列' });

    let changed = 0;
    const fromTrim = fromRaw.trim();
    for (const row of data.rows) {
      const key = data.headers[idxDate];
      const old = row[key] != null ? String(row[key]).trim() : '';
      if (!old) continue;
      // 既支持直接文本匹配，也支持把 3.14 / 2026-03-14 这类规范后再比较
      const oldComparable = dateToComparable(old);
      const fromComparable = dateToComparable(fromTrim);
      if (old === fromTrim || (oldComparable && fromComparable && oldComparable === fromComparable)) {
        row[key] = toRaw;
        changed += 1;
      }
    }
    if (!changed) {
      return res.json({ ok: true, message: '没有找到需要修改的记录', changed: 0 });
    }
    saveSmartTable(data);
    rebuildAnalyzedFromSmartTable();
    res.json({ ok: true, message: `已把 ${changed} 条记录的日期从「${fromRaw}」改成「${toRaw}」`, changed });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
