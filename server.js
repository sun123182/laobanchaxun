...
function rebuildAnalyzedFromSmartTable() {
  ...
  rows = rows.map((r) => {
    ...
    const newDate = canonical ? (toMonthDotDay(canonical) || canonical) : (r.date != null ? String(r.date).trim() : '');
  ...
  try {
    ...
    if (body.text && typeof body.text === 'string') {
      ...
      toAppend = analyzed.rows.map((r) => ({
        date: toMonthDotDay(r.date) || r.date || '',
        roleName: r.roleName || '未知',
...
app.post('/callback', async (req, res) => {
  try {
    ...
    try {
      ...
      // 写入智能表格的日期用「月.日」格式（如 3.13、3.1），按实时中国时区
      const chinaTz = 'Asia/Shanghai';
      const now = new Date();
