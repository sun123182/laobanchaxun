...
/**
 * 仅根据智能表格重算分析结果并落盘（供前端展示）。
 * 只做两件事：把每行日期规范成 YYYY-MM-DD（Dxx、无效日修正），再去重；有变更才写回智能表格。不碰回调写入逻辑。
 */
function rebuildAnalyzedFromSmartTable() {
  ...
  let changed = false;
  rows = rows.map((r) => {
    const canonical = dateToDedupeKey(r.date);
    const newDate = canonical || (r.date != null ? String(r.date).trim() : '');
    if (newDate && newDate !== (r.date != null ? String(r.date).trim() : '')) {
      changed = true;
  ...
  if (changed) {
    ...
    if (rows.length < beforeCount)
      console.log('[smart-table] 重建分析：去重并规范日期，删除', beforeCount - rows.length, '条重复');
    else console.log('[smart-table] 重建分析：已规范日期并保存');
  }
  let totalDiff = 0;
  const byDate = {};
  const byRole = {};
