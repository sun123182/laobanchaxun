...
/** 将表格单元格中的日期规范为 YYYY-MM-DD；仅数字 1-31 时视为“仅日”存为 D01..D31，避免与真实日期混算 */
...
/** 去重时统一日期：D12 与 2026-03-12 视为同一天，避免“日12(仅日)”和“2026-03-12”同一条记两遍 */
function dateToDedupeKey(dateStr) {
  ...
  if (part) return `${part[1]}-${String(parseInt(part[2], 10)).padStart(2, '0')}-${String(parseInt(part[3], 10)).padStart(2, '0')}`;
  return s;
}
...
/** 用服务器智能表格的 rows 重算并写入 analyzed，使「可分析表格」实时与智能表格一致；先自动去重再保存 */
function rebuildAnalyzedFromSmartTable() {
  ...
  if (rows.length < before) {
    saveSmartTable(rows);
    console.log('[smart-table] 自动去重，删除', before - rows.length, '条重复');
  }
  let totalDiff = 0;
  const byDate = {};
  const byRole = {};
...
app.post('/callback', async (req, res) => {
  try {
    ...
    try {
      ...
      // 写入服务器智能表格，供网页实时查看与分析
      const today = new Date();
      const dateStr = today.getFullYear() + '-' + String(today.getMonth() + 1).padStart(2, '0') + '-' + String(row.date != null ? row.date : today.getDate()).padStart(2, '0');
      const smartRow = {
        date: dateStr,
