...
app.post('/api/smart-table/import', requireAdmin, express.json({ limit: '6mb' }), (req, res) => {
  try {
    ...
    if (body.text && typeof body.text === 'string') {
      ...
      return res.json({ ok: true, message: '已导入到智能表格（与粘贴内容完全一致）', imported: rows.length, recordCount: rows.length, columns: headers.length });
    }
...
app.post('/api/smart-table/import-upload', requireAdmin, (req, res, next) => {
  upload.single('file')(req, res, (err) => {
    ...
    try {
      ...
      sendJson(res, 200, { ok: true, message: '已导入到智能表格（与文件完全一致）', imported: rows.length, recordCount: rows.length, columns: headers.length });
    } catch (e) {
